...
export default async function Compare({ params }: { params: Promise<{ id: string }> }) {
  ...
  const rows = compareChronicleEvents(chronicleEvents(original), chronicleEvents(newer));
  ...
  return <main id="main" className="shell inner-page compare-page">
    ...
    <header className="page-heading compare-heading"><span className="eyebrow">TWO EDITIONS, ONE YEAR</span><h1>{year}<span>年大事记 · 版本对比</span></h1><p>同一段历史，两版记述。按月份并排阅读，保留各版原有分段。</p></header>
    ...
    <div className="comparison-scroll" role="region" aria-label={`${year} 年两个版本的并排正文`} tabIndex={0}>
      <div className="comparison-grid" id="article-body">
        ...
        {rows.map((row, index) => <section className="comparison-period" id={`compare-period-${index + 1}`} key={index} aria-labelledby={`compare-period-title-${index + 1}`}>
          ...
          <div className="comparison-columns">
            {(['left', 'right'] as const).map((side, column) => <div className="comparison-cell prose" data-edition={column === 0 ? '2009' : '2019'} aria-label={`${column === 0 ? '2009' : '2019'} 年版 · ${row.label}`} key={side}>
              {row[side].length ? row[side].map(event => <p className="chronicle-event" id={`${column === 0 ? '2009' : '2019'}-${event.id}`} key={event.id}>{event.text}</p>) : <p className="comparison-empty">本版此处无记事</p>}
            </div>)}
          </div>
        </section>)}
      </div>
